...
def capture_elements_per_day(time_grid, day_beginnings, aggregated, aggregation_factor=1):
    """
    Captures the elements in the interval of each day based on the day beginnings.
...
class FluxModel:
    ...
    def __init__(self, travel_rates, T, save_path, aggregated, aggregation_factor):
        ...
        self.day_elements = capture_elements_per_day(self.T, self.day_beginnings, self.aggregated, self.aggregation_factor)
    ...
    def _serial_ode(self, N, t):
        ...
        """
        print(t)
        print("temp ->", temporal_verifier(self.day_elements, t, self.step))
...
def generate_integration_scenarios(travel_rates, T, save_path, transition_rates):
    ...
    for index, aggregated_network in enumerate(aggregated_networks):
        model = SIRModel(aggregated_network, (time_grid, step), save_path, beta, gamma, True, resolutions[index])
        trajectory = model.Y
