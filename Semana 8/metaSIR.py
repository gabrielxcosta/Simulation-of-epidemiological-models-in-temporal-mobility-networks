...
def compare_trajectories(baseline_trajectory, other_trajectories):
    ...
    Parameters:
        baseline_trajectory (numpy.ndarray): The trajectory of the baseline scenario (higher resolution - 1 network per day).
            It should be a 2D numpy array with shape (T, 3 * K), where T is the number of time steps and K is the number
            of subpopulations * 3 Compartments of the SIR Model.
        other_trajectories (dict): A dictionary containing SIRModel objects for different aggregation scenarios.
            The keys represent the aggregation factors, and the values are corresponding SIRModel objects.

    Returns:
        dict: A dictionary containing comparison metrics for each trajectory compared to the baseline trajectory.
            The keys are the aggregation factors, and the values are dictionaries with the following metrics:
...
def temporal_verifier(arrays, value, dt, aggregated):
    """
...
class FluxModel:
    ...
    def _serial_ode(self, N, t):
        ...
        """
        print('FLUX')
        print('PONTO CORRENTE:', t)
        print("DIA: ", temporal_verifier(self.day_elements, t, self.step, self.aggregated) + 1)
        outflow = self.outflow_rates[temporal_verifier(self.day_elements, t, self.step, self.aggregated)]
        inflow = self.inflow_rates[temporal_verifier(self.day_elements, t, self.step, self.aggregated)]
    ...
    def plot_diffusion(self):
        ...
        if not self.aggregated:
            ...
            return fig.savefig(os.getcwd() + '/' + self.save_path + f'/flux_model_results_aggregated_{60 / self.aggregation_factor}.pdf')
...
def generate_integration_scenarios(travel_rates, T, save_path, transition_rates):
    ...
    for index, aggregated_network in enumerate(aggregated_networks):
        ...
        #resolution = model.K  # Assuming model.K represents the resolution
