...
def compare_trajectories(trajectories_path):
    """
    Compare the trajectories of all the nodes using the Frobenius norm and Pearson Correlation.
...
def generate_equal_travel_rates(N, save_path, days):
    """
    Generates NxN matrices of travel rates between N locations for multiple days,
    where each column represents the travel rates from one location to all other locations,
    with 100% equally divided among the locations.
    ...
    for day in range(days):
        ...
        # Calculate the equal distribution value
        equal_distribution = 100.0 / (N - 1)

        for j in range(N):
            # Set the jth column in inflow matrix to equal_distribution, excluding the diagonal
            inflow_matrix[:, j] = equal_distribution
            inflow_matrix[j, j] = 0.0

            # Set the jth row in outflow matrix to equal_distribution, excluding the diagonal
            outflow_matrix[j, :] = equal_distribution
            outflow_matrix[j, j] = 0.0
...
def generate_random_travel_rates(N, save_path, days):
    """
...
class SIROrdinary:
    """
    A class for simulating the Susceptible-Infected-Recovered (SIR) model for a single population using ordinary differential equations (ODEs).
    This class models the spread of infectious diseases in a single population. It computes the dynamics of the SIR model using ordinary differential 
    equations (ODEs).

    Parameters:
    ----------
    T : tuple
        A tuple containing the time grid and step size. The first element is a 1D numpy array specifying the time points at which
        the simulation should be run. The second element is a float representing the step size between time points.
    save_path : str
        The path to save the simulation results.
    beta : float
    ...
    K : int
        The number of total cities/subpopulations.
    
    Attributes:
    ----------
    T : tuple
        A tuple containing the time grid and step size.
    save_path : str
        The path to save the simulation results.
    beta : float
        The transmission rate of the disease.
    ...
    K : int
        The number of total cities/subpopulations.
    Y0 : numpy.ndarray
        A 1D array representing the initial conditions for the differential equations.
    Y : numpy.ndarray or None
        An array of dimension Tx3 that stores the population size of the single population at each time step.
    basic_reproduction_number : float
        The basic reproduction number of the SIR model.
    """
    def __init__(self, T, save_path, beta, gamma, K):
        """
        Initializes a new instance of the SIROrdinary class with the given parameters and runs the simulation.

        Args:
            T (tuple): A tuple containing the time grid and step size.
            save_path (str): The path to save the simulation results.
            beta (float): The transmission rate of the disease.
            gamma (float): The recovery rate of the disease.
            K (int) : The number of total cities/subpopulations.
        """
        self.T, self.step = T
        self.total_days = int(self.T[-1])
        self.save_path = save_path
        ...
        self.K = K
        self.N = self.K * 500
        self.Y0 = np.array([self.N - (100 * self.K), self.N - (400 * self.K), 0])
        self.Y = None
    ...
    def _serial_system_ode(self, y, t):
        """
        Computes the system of ordinary differential equations (ODEs) for the SIR model.

        Args:
            y (numpy.ndarray): A 1D array with length 3 that specifies the initial values of the SIR model variables.
                The first element is the initial susceptible population, the second element is the initial infected population,
                and the third element is the initial recovered population.
            t (float): Current time point.

        Returns:
            numpy.ndarray: A 1D array that specifies the values of the derivatives of the SIR model variables at each time point.
        """
        S = y[0]
        I = y[1] 
        R = y[2]
    ...
    def run_simulation(self):
        """
        Runs the simulation of the SIR model using ordinary differential equations (ODEs).
        The results of the simulation are stored in the 'Y' attribute.
        """
        def save_integration_csv(self):
            """
            ...
            header = [f'{status} {i + 1}' for status in ('Susceptible', 'Infected', 'Removed') for i in range(1)]
            data = np.column_stack((S, I, R))

            dir = os.path.join(self.save_path, "SIROrdinary_integration")
            if not os.path.exists(dir):
                os.makedirs(dir)
            file_path = os.path.join(dir, "SIROrdinary_integration.csv")
        ...
        save_integration_csv(self)
        print('SIR ORDINARY - COMPLETED')
    
    def R_0(self):
        """
        Calculates the basic reproduction number (R0) of the SIR model.

        Returns:
            float: The value of the basic reproduction number (R0).
        """
        return self.beta / self.gamma
    
    def plot_system_ode(self):
        """
        Plots the results of the SIR model simulation and saves the graph to a PDF file.

        The graph displays the curves for Susceptible (S), Infected (I), and Recovered (R) as functions of time.
        ...
        fig.suptitle(f'Single Subpopulation - Ordinary SIR Model', fontsize=18)
        ...
        ax.plot(self.T, self.Y[:, 0], 'blue', lw=3, alpha=.7, label=r"Susceptible - $S(t)$")
        ax.plot(self.T, self.Y[:, 1], 'red', lw=3, alpha=.7, label=r"Infected - $I(t)$")
        ax.plot(self.T, self.Y[:, 2], 'green', lw=3, alpha=.7, label=r"Recovered - $R(t)$")
        ...
        dir = os.path.join(self.save_path, f"SIROrdinary_integration")
        fig.savefig(os.path.join(dir, f"SIROrdinary_integration.pdf"))
        plt.close(fig)
...
def generate_integration_scenarios(mode, travel_rates, T, save_path, transition_rates):
    """
    Generate integration scenarios for the SIR model with different temporal resolutions or network states.

    Parameters
    ----------
    mode : str
        The mode of operation, which can be 'baseline/aggregated' or 'with_without_net'.
    travel_rates : tuple(numpy.ndarray, numpy.ndarray)
        The tuple of inflow and outflow numpy.ndarrays that specifies the rates of movement between
        subpopulations. The first numpy.ndarray is the inflow rates, and the second numpy.ndarray is the outflow rates.
    T : tuple(numpy.ndarray, float) 
    ...
    transition_rates : tuple(float, float)
        A tuple containing the transition rates for the SIR model. The first element is a float specifying the beta
        (transmission rate). The second element is a float representing the gamma (recovery rate) for the infection.

    Returns
    -------
    dict
        A dictionary containing comparison results for different scenarios. If the mode is 'baseline/aggregated', the
        keys are the resolutions, and the values are normalized Frobenius norms comparing the trajectories with the baseline.
        If the mode is 'with_without_net', the keys are 'with_net' and 'without_net', and the values are the respective
        SIR model trajectories.
    """
    trajectories = {}
    beta, gamma = transition_rates
    K = len(travel_rates[0][0])

    if mode == 'baseline/aggregated':
        resolutions = [i for i in range(60, 0, -1) if 60 % i == 0][1:]
        baseline_model = SIRModel(travel_rates, T, save_path, np.ones(K) * beta, gamma, False, None)
        ...
        for index, aggregated_network in enumerate(aggregated_networks):
            model = SIRModel(aggregated_network, T, save_path, np.ones(K) * beta, gamma, True, 60 // resolutions[index])
            trajectory = model.Y
    ...
    elif mode == 'with_without_net':
        without_net = SIROrdinary(T, save_path, beta, gamma, K)
        with_net = SIRModel(travel_rates, T, save_path, np.ones(K) * beta, gamma)
        trajectories['with_net'] = with_net.Y
        trajectories['without_net'] = without_net.Y

        with open(os.getcwd() + '/' + save_path + '/with_without_trajectories.pkl', 'wb') as file:
            pk.dump(trajectories, file)
...
def extract_subpopulation_norms(trajectories):
    """
    Extract subpopulation norms from a dictionary or ndarray of integration results.
    ...
    if isinstance(trajectories, dict):
        ...
        subtrajectories = {'with_net' : {}}
        dim_1, dim_2 = trajectories.shape
        dim_3 = dim_2 // 303
        dim_4 = dim_2 // dim_3
        for i in range(dim_4):
            with_net_trajectory = trajectories
            with_net_subtrajectory = np.empty((dim_1, dim_3))
            with_net_subtrajectory[:, 0] = with_net_trajectory[:, i]
            with_net_subtrajectory[:, 1] = with_net_trajectory[:, dim_4 + i]
            with_net_subtrajectory[:, 2] = with_net_trajectory[:, (dim_4 * 2) + i]
            subtrajectories['with_net'][i] = frobenius_norm(with_net_subtrajectory)
        
        with open('with_net_subtrajectories_norms.pkl', 'wb') as file:
            pk.dump(subtrajectories, file)
...
def calculate_node_centrality(inflow_matrix, outflow_matrix, city_names):
    """
    Calculate node centrality for a single day's inflow and outflow matrices.
    ...
    -------
    dict
        A dictionary containing the node centrality for inflow and outflow for that day.
    """
    inflow_graph = nx.DiGraph(inflow_matrix)
    outflow_graph = nx.DiGraph(outflow_matrix)

    inflow_centrality = nx.degree_centrality(inflow_graph)
    outflow_centrality = nx.degree_centrality(outflow_graph)

    inflow_centrality_by_city = {city_names[i]: centrality for i, centrality in enumerate(inflow_centrality)}
    outflow_centrality_by_city = {city_names[i]: centrality for i, centrality in enumerate(outflow_centrality)}

    return {
        'inflow': inflow_centrality_by_city,
        'outflow': outflow_centrality_by_city
    }
...
def calculate_centralities(travel_rates):
    ...
    for t in range(T):
        ...
        results.append(pool.apply_async(calculate_node_centrality, args=(inflow_matrix, outflow_matrix, city_names)))
