...
def generate_travel_rates(N, save_path):
    """
    Generates an NxN matrix of travel rates between N locations, where each column
...
class FluxModel:
    ...
    travel_rates : tuple(numpy.ndarray, numpy.ndarray)
        ...
        output : dict or None
            ...
            to None initially.
        limiting_population_sizes : numpy.ndarray or None
            An N-element array that specifies the limiting population sizes for each subpopulation. It is set to None initially.
...
class SIRModel(FluxModel):
    ...
    def R_0(self):
        """
        Calculates the basic reproduction number (R₀) for a disease using the Next-generation matrix approach.
